({
    initializeRetailer : function(component, event, helper) {
        var action=component.get("c.getWarehouseData");
        action.setCallback(this,function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                for (var i = 0; i < result.length; i++) {
                    var row = result[i];
                    if (row.Warehouse_name_holder__c) row.Warehouse_name_holder__c = row.Warehouse_name_holder__c;
    ...
    searchHelper : function(component,event,getInputkeyWord) {
        ...
        var action = component.get("c.fetchLookupWarehousedataList"); 
        action.setParams({
